import {
    ClaimWithRemaining,
    QueryOptions,
    type QueryResult,
} from "../../types";

export interface UseClaimsMvmParams extends QueryOptions<
    ClaimWithRemaining[] | undefined
> {
    address?: string;
}

export type UseClaimsMvmReturnValue = QueryResult<
    ClaimWithRemaining[] | undefined
>;

export function useClaimsMvm(
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _params: UseClaimsMvmParams,
): UseClaimsMvmReturnValue {
    // TODO: not implemented yet, returns an idle result so the useClaims
    // dispatcher can call it unconditionally
    return {
        data: undefined,
        isPending: false,
        isLoading: false,
        isFetching: false,
    };
}
