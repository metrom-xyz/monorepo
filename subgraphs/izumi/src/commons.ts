...
export const BI_MAX_LOG_INDEX = BigInt.fromI32(2).pow(20 as u8);
...
export const LiquidityManagerContract = LiquidityManager.bind(
    LIQUIDITY_MANAGER_ADDRESS,
);
export const FactoryContract = Factory.bind(FACTORY_ADDRESS);

export function getEventId(blockNumber: BigInt, logIndex: BigInt): Bytes {
    return changetype<Bytes>(
        blockNumber.leftShift(40).plus(logIndex).reverse(),
    );
}
...
export function updatePoolState(
    ...
    if (newTick != pool.tick || newSqrtPriceX96 != pool.sqrtPriceX96) {
        let swapChange = new SwapChange(
            getEventId(blockNumber, BI_MAX_LOG_INDEX),
        );
        swapChange.timestamp = blockTimestamp;
        swapChange.blockNumber = blockNumber;
        swapChange.pool = pool.id;
        swapChange.tick = newTick;
        swapChange.sqrtPriceX96 = newSqrtPriceX96;
        swapChange.save();
    }
    ...
    pool._stateUpdateBlockNumber = blockNumber;
    pool.save();
}
