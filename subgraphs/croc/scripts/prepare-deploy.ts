...
const ammConfig = chainConfig[amm];
if (!ammConfig) {
    console.error(
        `"${amm}" is not a valid network. Valid values for chain ${network} are: ${Object.keys(chainConfig).join(", ")}`,
    );
    process.exit(1);
}
...
addressesFile += `export const CROC_QUERY_ADDRESS = Address.fromString("${ammConfig.contracts.CrocQuery.address}");\n`;
addressesFile += `export const NATIVE_TOKEN_ADDRESS = Address.fromString("${ammConfig.nativeToken.address}");\n`;
addressesFile += `export const NATIVE_TOKEN_SYMBOL = "${ammConfig.nativeToken.symbol}";\n`;
addressesFile += `export const NATIVE_TOKEN_NAME = "${ammConfig.nativeToken.name}";\n`;
addressesFile += `export const NATIVE_TOKEN_DECIMALS = BigInt.fromI32(${ammConfig.nativeToken.decimals});\n`;
...
try {
    ...
    writeFileSync(
        ...
        Mustache.render(
            ...
            {
                network,
                dexAddress: ammConfig.contracts.CrocSwapDex.address,
                dexStartBlock: ammConfig.contracts.CrocSwapDex.startBlock,
            },
        ),
    );
