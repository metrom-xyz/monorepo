...
if (!contracts) {
    console.error(
        `"${amm}" is not a valid Liquity fork for the network ${network}. Valid values are: ${Object.keys(networkConfig).join(", ")}`,
    );
    process.exit(1);
}
        ...
        Mustache.render(
            ...
            ).toString(),
            {
                ...
                poolAddressesProviderRegistryStartBlock:
                    contracts.PoolAddressesProviderRegistry.startBlock,
            },
        ),
    );
